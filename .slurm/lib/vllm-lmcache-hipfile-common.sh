...
#
# Shared helpers for vllm-lmcache-hipfile Slurm jobs (sourced by vllm-lmcache-hipfile.sbatch).
...
_vlh_gutenberg_prereqs() {
    ...
    if ! _vlh_gutenberg_fixtures_present "${GUTENBERG_DATA_ROOT}"; then
        ...
        _vlh_log "  make -C ${RECIPE_DIR} data-all BOOK_DATA_ROOT=\${VLH_GUTENBERG_DATA_ROOT}"
        return 1
    fi
...
_vlh_run_test_aic() {
    ...
    docker exec "${CONTAINER_NAME}" python3 /app/scripts/test-aic.py \
        --json -o "/var/log/vllm-lmcache-hipfile/test_aic.json" \
        "${_aic_extra[@]}" \
        > "${out}" 2>&1 || true
    docker cp "${CONTAINER_NAME}:/var/log/vllm-lmcache-hipfile/test_aic.json" "${out}" 2>/dev/null || true
}
...
_vlh_summarize_job() {
    local script="${SLURM_LIB}/summarize-vllm-lmcache-hipfile-job.py"
    if [[ ! -f "${script}" ]]; then
    ...
    if python3 "${script}" "${REPORT_DIR}"; then
        ...
        echo ""
        echo "========== vllm-lmcache-hipfile results summary =========="
        sed -n '1,80p' "${REPORT_DIR}/results-summary.md" 2>/dev/null || true
        echo "================================================="
        echo ""
    else
        _vlh_log "WARN: summarize-vllm-lmcache-hipfile-job.py failed"
    fi
}

...
- **2026-07-15 01:49 EDT** — job `67534362` **COMPLETED** (vram_only ok; nvme ok
  ...
  same config but **`AIC_NIXL_BUFFER_SIZE=8 GiB`** (up from 512 MiB) to survive
...
**Fix (re-run):** raise `AIC_NIXL_BUFFER_SIZE` 512 MiB → **8 GiB** (VRAM is
...
- **2026-07-15 13:15 EDT** — **NEW run `67535846`** queued with the open-item fix
  ...
  `AIC_KV_LOAD_FAILURE_POLICY`), plus NVMe pool **65536**, NIXL buffer **8 GiB**,
...
- Keep 8 GiB NIXL staging buffer. **Leave `AIC_KV_LOAD_FAILURE_POLICY` unset**
...
AIC_CLIFF_ARMS=vram,nvme \
AIC_LOCAL_CPU=true LMCACHE_MAX_LOCAL_CPU_SIZE=128 \
LMCACHE_NVME_POOL=131072 AIC_NIXL_BUFFER_SIZE=8589934592 \
BENCH_ITERS=3 AIC_CLIFF_TIME=08:00:00 \
make cliff-submit
```

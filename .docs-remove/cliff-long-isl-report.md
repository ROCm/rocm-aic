...
resubmitted again with a **3 h wall** (`AIC_CLIFF_TIME=03:00:00`) — the 16 h default
...
order **vram → nvme → gds** (`AIC_CLIFF_ARMS` only selects, doesn't reorder — so gds
...
1. **Confirm first (cheap):** run nvme arm **NVMe-only** past the cliff —
   `AIC_CLIFF_ARMS=nvme AIC_LOCAL_CPU=false BENCH_CONCUR=1,16,32 make cliff-long-64k`.
...
only substitutes when unset/empty) → it silently kept `4096`. `AIC_NIXL_BUFFER_SIZE`
...
the env reaching `sbatch` carried `LMCACHE_NVME_POOL=4096` + `AIC_NIXL_BUFFER_SIZE=

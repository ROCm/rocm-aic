...
2. rocm-aic image override (via `--set decode.containers[0].image=...`)
...
- ✅ Preserves rocm-aic customization flexibility
...
3. Apply rocm-aic base overrides (vLLM image)
...
**Note**: This deployment uses the llm-d guide's `helmfile.yaml.gotmpl` directly to minimize code duplication while allowing rocm-aic specific customizations through values files.
...
rocm-aic uses a **layered values approach** to minimize duplication while allowing customization:
...
#### Layer 2: rocm-aic Base (always applied)
...
```bash
# Use llm-d defaults + rocm-aic base only
just deploy

# Use llm-d defaults + rocm-aic base + production overrides
VALUES_FILE=amd-default.yaml just deploy

# Use llm-d defaults + rocm-aic base + testing overrides
...
Configuration:
- Image: `vllm/vllm-openai-rocm:v0.17.0` (rocm-aic override)
...
**Note**: Values are layered (llm-d defaults → rocm-aic-base → your custom values), so you only need to specify what changes.

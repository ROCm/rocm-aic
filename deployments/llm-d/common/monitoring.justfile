...
# Import this file in deployment-specific justfiles with: import? "../../common/monitoring.justfile"
...
_parse-vllm-pod NAMESPACE POD_NAME:
    @python3 ../../common/parse-vllm-config.py -n {{NAMESPACE}} -p {{POD_NAME}}
...
_parse-vllm-all NAMESPACE LABEL_SELECTOR="llm-d.ai/inference-serving=true" DEDUPE="true":
    #!/usr/bin/env bash
    if [ "{{DEDUPE}}" = "true" ]; then
        python3 ../../common/parse-vllm-config.py -n {{NAMESPACE}} -l {{LABEL_SELECTOR}} --deduplicate
    else

...
Do **not** use `AAI_KV_LOAD_FAILURE_POLICY=recompute` with async scheduling on
vLLM v0.25.0.
...
`V1 LLM engine (v0.25.0)` (aai-day-release image). File paths above are from that
...
- **Don't pair `recompute` with async scheduling** (current guidance). Leave
  `AAI_KV_LOAD_FAILURE_POLICY` unset (vLLM default `fail`, which degrades to
...
- **To use `recompute` gracefully**, drop `--async-scheduling` (no placeholders to
  underflow) — untested here; a cheap experiment (would need an `AAI_ASYNC_SCHEDULING`

...
`patches/vllm/04-llm-emu-emulator-hooks.patch`.
...
> a request completes. `patches/vllm/04-llm-emu-emulator-hooks.patch` answers
...
| `Qwen-Qwen2.5-3B-Instruct-20260902-182346.json` | AMD Radeon RX 9070 XT (gfx1201) | `Qwen/Qwen2.5-3B-Instruct` |
...
**On a Slurm/SPUR cluster** (submits an sbatch job):
...
**Locally** (any host with `/dev/kfd` — no cluster needed):

```bash
make capture-profile-local \
  IMAGE_REF=<built-image> \
  ROCM_ARCH=gfx1201 \
  AIC_CAPTURE_MODEL=Qwen/Qwen2.5-3B-Instruct \
  HF_TOKEN_FILE=~/.cache/huggingface/token
# pack + trace + bench JSONs land in profiles/captures/
```

The local target uses the same flow as the cluster path but runs `docker run`
directly instead of submitting to sbatch. Output lands in `AIC_CAPTURE_DIR`
(default `profiles/captures/`). See `make help` for all knobs.

Both paths run a **real serve on a real GPU** with `VLLM_EMULATOR_TRACE_STEP_CYCLE=1`
